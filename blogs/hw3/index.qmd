...
def neg_log_likelihood(beta, X, y, group_ids):
    ...
    X: matrix of covariates (n_obs x n_features)
    y: binary array indicating chosen alternative
    group_ids: identifiers for choice tasks
    """
    linear_part = X @ beta
    log_probs = linear_part - logsumexp(linear_part.reshape(-1, 1), axis=0, where=group_ids.reshape(-1, 1)==group_ids.reshape(1, -1))
    return -np.sum(log_probs[y == 1])

# Step 2: Prepare matrices
features = ['price', 'brand_Prime', 'brand_Hulu', 'ad_Yes']
X = X[features].values
y = X_data['choice'].values
groups = X_data['resp'].astype(str) + '_' + X_data['task'].astype(str)

# Convert group labels to numeric IDs
group_ids = pd.factorize(groups)[0]

# Step 3: Estimate using scipy.optimize
init_beta = np.zeros(X.shape[1])
res = minimize(neg_log_likelihood, init_beta, args=(X, y, group_ids), method='BFGS')

# Step 4: Get parameter estimates and standard errors
beta_hat = res.x
...
import ace_tools as tools; tools.display_dataframe_to_user(name="MLE Results", dataframe=results)
```
