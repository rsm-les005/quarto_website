...
They studied the effects of matching grants on donation behavior. The experiment used direct mail sent to 50,083 prior donors of a politically progressive nonprofit, randomly assigning recipients to a control or one of several treatment groups. The treatment letters varied in terms of the match ratio offered (1:1, 2:1, 3:1), the maximum match threshold, and the suggested donation amount. The key findings were that the presence of a matching grant increased both the response rate and average donation, but increasing the match ratio did not significantly increase contributions further.
...
We use the dataset from Karlan and List's 2007 study with 50,083 observations. Key variables include:
- `gave`: Whether the individual donated (1) or not (0)
- `amount`: Dollar amount donated
- `treatment`: Indicator for receiving a matching grant offer
- `control`: Indicator for receiving a control letter
- `ratio`: The match ratio offered (1, 2, or 3)
- `redcty` and `bluecty`: Whether the donor lives in a red or blue county based on 2004 US presidential election
